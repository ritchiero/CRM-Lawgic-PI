...
// Check if one name is a subset of another (e.g., "Juan Carlos" vs "Juan Carlos Lopez")
...
export default function DuplicatesPage() {
          ...
          for (let j = i + 1; j < prospects.length; j++) {
                    ...
                    if (!norm2) continue;
                                                ...
                                                if (norm1 === norm2) {
                                                            group.push(prospects[j]);
                                                            processed.add(prospects[j].id);
                                                            matchType = matchType === 'exact' ? 'case-insensitive' : matchType;
                                                            continue;
                                                }

                                                // Name subset check (e.g., "Hector Ancona" vs "Hector Carlos Ancona Torres")
                                                if (isNameSubset(norm1, norm2)) {
                                                            group.push(prospects[j]);
                                                            processed.add(prospects[j].id);
                                                            matchType = 'partial-name';
                                                            maxSim = Math.min(maxSim, similarity(norm1, norm2));
                                                            continue;
                                                }
  ...
  // Merge duplicates: keep the master, update targets, delete others
  const handleMerge = useCallback(async (groupKey: string, keepId: string, deleteIds: string[]) => {
        ...
        try {
                ...
                if (!keeper) throw new Error('Prospect to keep not found');

          // 1. Merge data from duplicates into the keeper (fill empty fields)
          ...
          for (const dup of toDelete) {
                  ...
                  if (keeper.notes && dup.notes && keeper.notes !== dup.notes) {
                              mergedData.notes = keeper.notes + '\n---\n' + dup.notes;
                  }
          }

          // Update the keeper with merged data
          if (Object.keys(mergedData).length > 0) {
                    await updateProspect(keepId, mergedData);
          }

          // 2. Update targets that reference the deleted prospect IDs
          for (const delId of deleteIds) {
                    const targetsQuery = query(
                                collection(db, 'targets'),
                                where('copiedFromProspectId', '==', delId)
                              );
                    ...
                    for (const targetDoc of targetsSnapshot.docs) {
                                await updateDoc(doc(db, 'targets', targetDoc.id), {
                                              copiedFromProspectId: keepId,
                                });
                    }
          }

          // 3. Delete the duplicate prospects
          for (const delId of deleteIds) {
                    await deleteProspect(delId);
          }

          setMergeResults(prev => [...prev, {
                    group: groupKey,
                    success: true,
                    message: `Merged ${deleteIds.length} duplicate(s). Kept "${keeper.name}". Updated ${deleteIds.length} target references.`
          }]);
        } catch (error) {
  ...
  const getStageLabel = (stage: string) => {
        const labels: Record<string, string> = {
                'Detecci\u00f3n de prospecto': 'Detecci\u00f3n',
                '1er Contacto': '1er Cont.',
                'Contacto efectivo': 'Efectivo',
                'Muestra de inter\u00e9s': 'Inter\u00e9s',
                'Cita para demo': 'Cita Demo',
                'Demo realizada': 'Realizada',
                'Venta': 'Venta',
                'En Pausa': 'Pausa',
                'Basura': 'Basura',
  ...
  const getMostCompleteProspect = (group: Prospect[]): string => {
        ...
        for (const p of group) {
                ...
                if (p.email) score += 2;
                if (p.phone) score += 2;
                if (p.company) score += 1;
                if (p.notes) score += 1;
                if (p.linkedinUrl) score += 1;
                if (p.potentialValue) score += 1;
                if (p.accountValue) score += 1;
                if (p.brandCount) score += 1;
                // Prefer more advanced stages
        ...
        <ProtectedRoute>
              <div style={{ minHeight: '100vh', backgroundColor: 'var(--background)', fontFamily: 'var(--font-sans)', padding: '2rem' }}>
                      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
                        {/* Header */}
                                <div style={{ marginBottom: '2rem' }}>
                                            ...
                                            <h1 style={{ fontSize: '2rem', fontWeight: '700', color: 'var(--foreground)', margin: 0 }}>
                                                          Gesti&oacute;n de Duplicados
                                            </h1>h1>
                                            <p style={{ fontSize: '0.875rem', color: 'var(--secondary)', marginTop: '0.5rem' }}>
                                                          Detecta y fusiona prospectos duplicados de forma segura. Los targets se actualizan autom&aacute;ticamente.
                                            </p>p>
                                </div>div>
                      
                        {/* Stats Bar */}
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
                                            <div style={{ padding: '1rem 1.25rem', backgroundColor: 'var(--surface)', borderRadius: '0.75rem', border: '1px solid var(--border)' }}>
                                                          <div style={{ fontSize: '0.75rem', color: 'var(--secondary)', marginBottom: '0.25rem' }}>Total prospectos</div>div>
                                                          <div style={{ fontSize: '1.5rem', fontWeight: '700', color: 'var(--foreground)' }}>{prospects.length}</div>div>
                                            </div>div>
                                            <div style={{ padding: '1rem 1.25rem', backgroundColor: 'var(--surface)', borderRadius: '0.75rem', border: '1px solid var(--border)' }}>
                                                          <div style={{ fontSize: '0.75rem', color: 'var(--secondary)', marginBottom: '0.25rem' }}>Grupos duplicados</div>div>
                                                          <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#ef4444' }}>{duplicateGroups.length}</div>div>
                                            </div>div>
                                            <div style={{ padding: '1rem 1.25rem', backgroundColor: 'var(--surface)', borderRadius: '0.75rem', border: '1px solid var(--border)' }}>
                                                          <div style={{ fontSize: '0.75rem', color: 'var(--secondary)', marginBottom: '0.25rem' }}>Registros duplicados</div>div>
                                                          <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#f59e0b' }}>{duplicateGroups.reduce((acc, g) => acc + g.prospects.length - 1, 0)}</div>div>
                                            </div>div>
                                            <div style={{ padding: '1rem 1.25rem', backgroundColor: 'var(--surface)', borderRadius: '0.75rem', border: '1px solid var(--border)' }}>
                                                          <div style={{ fontSize: '0.75rem', color: 'var(--secondary)', marginBottom: '0.25rem' }}>Fusiones realizadas</div>div>
                                                          <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#22c55e' }}>{mergeResults.filter(r => r.success).length}</div>div>
                                            </div>div>
                                </div>div>
                        ...
                        {/* Controls */}
                          ...
                          <button
                                              ...
                                              style={{
                                                                    ...
                                                                    fontSize: '0.8rem',
                                                                    fontWeight: sensitivityLevel === level ? '600' : '400',
                                                                    color: sensitivityLevel === level ? '#fff' : 'var(--secondary)',
                                                                    backgroundColor: sensitivityLevel === level ? '#6366f1' : 'transparent',
                                                                    border: 'none',
                                                                    borderRadius: '0.5rem',
                                                                    cursor: 'pointer',
                                                                    fontFamily: 'inherit',
                        ...
                        {/* Loading */}
                        {loading && (
                      <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--secondary)' }}>Cargando prospectos...</div>div>
                                )}
                      ...
                      <div style={{ padding: '3rem', textAlign: 'center', backgroundColor: 'var(--surface)', borderRadius: '0.75rem', border: '1px solid var(--border)' }}>
                                    ...
                                    <p style={{ color: 'var(--secondary)', fontSize: '0.875rem', marginTop: '0.5rem' }}>
                                      {!searchTerm && 'Prueba cambiando el nivel de sensibilidad a "Agresivo" para detectar m\u00e1s coincidencias.'}
                                    </p>p>
                      </div>div>
                                )}
                        ...
                        const suggestedKeep = getMostCompleteProspect(group.prospects);
                        const lastResult = mergeResults.find(r => r.group === group.key);
          
                        if (lastResult?.success) return null; // Hide merged groups
                        ...
                        return (
                                          <div key={group.key} style={{ backgroundColor: 'var(--surface)', borderRadius: '0.75rem', border: '1px solid var(--border)', overflow: 'hidden' }}>
                                                            ...
                                                            <div style={{ padding: '0.75rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid var(--border)', backgroundColor: badge.bg }}>
                                                                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                                                                                      ...
                                                                                                      <span style={{ fontWeight: '600', fontSize: '0.9rem', color: 'var(--foreground)' }}>
                                                                                                        {group.prospects.length} registros similares
                                                                                                        </span>span>
                                                                                                      <span style={{ padding: '0.15rem 0.5rem', borderRadius: '1rem', fontSize: '0.7rem', fontWeight: '600', color: badge.color, backgroundColor: badge.bg, border: '1px solid ' + badge.color + '40' }}>
                                                                                                        {badge.label}
                                                                                                        </span>span>
                                                                    ...
                                                                    <span style={{ fontSize: '0.75rem', color: 'var(--secondary)' }}>
                                                                                ...
                                                                                <button
                                                                                                        onClick={() => {
                                                                                                                                  const deleteIds = group.prospects.filter(p => p.id !== suggestedKeep).map(p => p.id);
                                                                                                                                  setConfirmDelete({ groupKey: group.key, keepId: suggestedKeep, deleteIds });
                                                                                                          }}
                                                                                                        ...
                                                                                                        style={{
                                                                                                                                  ...
                                                                                                                                  fontWeight: '600',
                                                                                                                                  color: '#fff',
                                                                                                                                  backgroundColor: merging === group.key ? '#9ca3af' : '#6366f1',
                                                                                                                                  border: 'none',
                                                                                                                                  borderRadius: '0.5rem',
                                                                                                                                  cursor: merging === group.key ? 'not-allowed' : 'pointer',
                                                                                                                                  fontFamily: 'inherit',
                                                                                                                                  display: 'flex',
                                                                                                                                  alignItems: 'center',
                                                                                                                                  gap: '0.4rem',
                                            ...
                                            {group.prospects.map((prospect) => {
                                                                ...
                                                                return (
                                                                                        <div key={prospect.id} style={{
                                                                                                                  ...
                                                                                                                  display: 'grid',
                                                                                                                  gridTemplateColumns: '2fr 1.5fr 2fr 1fr 0.5fr',
                                                                                                                  gap: '0.75rem',
                                                                                                                  alignItems: 'center',
                                                                                                                  backgroundColor: isKeeper ? '#f0fdf4' : 'transparent',
                                                                                          }}>
                                                                                                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                                                                                                  {isKeeper && <CheckCircleIcon style={{ width: '1rem', height: '1rem', color: '#22c55e', flexShrink: 0 }} />}
                                                                                                                                          <span style={{ fontSize: '0.875rem', fontWeight: isKeeper ? '600' : '400', color: 'var(--foreground)' }}>
                                                                                                                                            {prospect.name}
                                                                                                                                            </span>span>
                                                                                                                ...
                                                                                                                <div style={{ fontSize: '0.8rem', color: 'var(--secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                                                                                  {prospect.company || 'Sin empresa'}
                                                                                                                  </div>div>
                                                                                                                <div style={{ fontSize: '0.8rem', color: 'var(--secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                                                                                  {prospect.email || 'Sin email'}
                                                                                                                  </div>div>
                                                                                                                <div>
                                                                                                                                          <span style={{ padding: '0.15rem 0.45rem', borderRadius: '0.25rem', fontSize: '0.7rem', fontWeight: '500', color: '#6366f1', backgroundColor: '#6366f110' }}>
                                                                                                                                            {getStageLabel(prospect.stage)}
                                                                                                                                            </span>span>
                                                                                                                  </div>div>
                                                                                                                <div style={{ textAlign: 'right' }}>
                                                                                                                  {!isKeeper && (
                                                                                                                      <TrashIcon style={{ width: '0.9rem', height: '0.9rem', color: '#ef4444', opacity: 0.6 }} />
                                                                                                                    )}
                                                                                                                  </div>div>
                                                                                          </div>div>
                                                                                      );
                                            ...
                                            {lastResult && !lastResult.success && (
                                                                <div style={{ padding: '0.5rem 1.25rem', backgroundColor: '#fef2f2', fontSize: '0.8rem', color: '#ef4444' }}>
                                                                  {lastResult.message}
                                                                </div>div>
                                                            )}
                                          </div>div>
          ...
          {confirmDelete && (
                  <>
                            ...
                            <div style={{
                                ...
                                backgroundColor: 'var(--surface)', borderRadius: '1rem', padding: '2rem', maxWidth: '500px', width: '90%',
                                boxShadow: '0 25px 50px rgba(0,0,0,0.25)', zIndex: 1001,
                  }}>
                                  ...
                                  Confirmar fusi&oacute;n de duplicados
                                        </h3>h3>
                                        <p style={{ textAlign: 'center', fontSize: '0.875rem', color: 'var(--secondary)', marginBottom: '1.5rem' }}>
                                                      Se conservar&aacute; el registro principal y se eliminar&aacute;n {confirmDelete.deleteIds.length} duplicado(s).
                                                      Los datos faltantes se copiar&aacute;n al registro principal y los targets se actualizar&aacute;n autom&aacute;ticamente.
                    </p>
                                    ...
                                    <div key={id} style={{ fontSize: '0.8rem', color: '#ef4444' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
                                                      ...
                                                      <button
                                                                        onClick={() => handleMerge(confirmDelete.groupKey, confirmDelete.keepId, confirmDelete.deleteIds)}
